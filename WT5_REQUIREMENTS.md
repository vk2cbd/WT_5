...
WT5 is a clean rebuild of the WT4 prototype antenna controller. WT4 should remain
available as the reference implementation and field-tested prototype. WT5 should
...
WT5 should be split into clear modules. Suggested structure:
...
Long slews should not be opaque single calls. WT5 should prefer a segmented
...
WT5 must make East/West differences visible.
...
The WT5 GUI should be more aesthetically pleasing while remaining practical and
field-friendly.
...
WT5 needs both event logging and measurement logging.
...
WT5 should include a simulation backend before hardware tests.
...
1. Create WT5 skeleton and this requirements document.
...
16. Field-test against WT4 behaviours before relying on WT5 operationally.

## Non-Goals For Initial WT5
...
- Do not remove WT4 until WT5 has proven field behaviour.

## Open Questions

- Should WT5 support both RTL-SDR and SDRplay in the first clean rebuild, or
  defer SDRplay until WT5 is stable?
...
- Should WT5 store measurement metadata in a small SQLite database later, while
  still exporting CSV?
