#!/usr/bin/env python3
"""WT5 two-antenna safety/calibration GUI."""
...
class LimitsDialog(tk.Toplevel):
    def __init__(self, app: "WT5App") -> None:
        super().__init__(app)
        self.app = app
...
class EncodersDialog(tk.Toplevel):
    ...
    def set_position(self, name: str, axis: Axis) -> None:
        ...
        if not messagebox.askyesno(
            ...
            "This resets the WT5 software calibration offset for this axis to zero.",
            parent=self,
        ):
            return
...
class AntennaPanel(ttk.Frame):
    def __init__(self, master: tk.Misc, app: "WT5App", name: str, config: Optional[AntennaConfig] = None) -> None:
...
class PowerMeterPanel(ttk.LabelFrame):
    def __init__(self, master: tk.Misc, app: "WT5App") -> None:
...
class WT5App(tk.Tk):
    def __init__(self, config_path: str) -> None:
        super().__init__()
        self.title(f"WT5 Antenna Controller {APP_VERSION}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Launch WT5 two-antenna GUI.")
    parser.add_argument("--config", default="WT5.ini", help="Config file. Default: WT5.ini")
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    app = WT5App(args.config)
    try:
        app.mainloop()
